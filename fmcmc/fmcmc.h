...
struct Edge {
    ...
    Edge (int from, int to, int cap, bool has_con): from(from), to(to), flow(0), cap(cap), has_con(has_con), pherom(PHEROM_INIT) {}
};
...
struct Fmcmc { ///flux maxim de cost minim cu costuri concave pe muchii (si capacitati pe muchii).
    ...
    Fmcmc (int cnt_nodes, int node_start, int node_end): mt(DEBUG? 0: rd()), dist(0, 1), cnt_nodes(cnt_nodes), node_start(node_start), node_end(node_end) {
        neighs.resize(cnt_nodes);
    }

    void add_edge(int from, int to, int cap, bool has_con);
